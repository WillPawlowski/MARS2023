...
class ODriveInterfaceAPI(object):
    ...
    def connect(self, port=None, right_axis=1, timeout=30):
        ...
        try:
            self.driver = odrive.find_any(serial_number="207939834D4D",timeout=timeout, logger=self.logger) # "206430804648"
            self.axes = (self.driver.axis0, self.driver.axis1)
        except:
    ...
    def left_current(self):       return self.left_axis.motor.current_control.Id_measured  if self.left_axis and self.left_axis.current_state > 1 else 0.
    def right_current(self):      return self.right_axis.motor.current_control.Id_measured if self.right_axis and self.right_axis.current_state > 1 else 0.
